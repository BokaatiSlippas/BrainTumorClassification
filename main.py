...
def parse_image(image_path):
    ...
    label = tf.cast(label == class_names, tf.float32) # One hot encoded
    return image, label
...
def create_model():
    base_model = EfficientNetB0(
        include_top=False, # Don't want old FC layers at the end
        weights='imagenet',
        input_shape=(IMG_SIZE[0], IMG_SIZE[1], 3)
    )
    base_model.trainable = False  # Freeze pre-trained weights (might want to eventually unfreeze)
